...
def process_finetuning_tabak(payload: dict):
    ...
    for idx, record_id in enumerate(record_ids):
        ...
        # Extract the base name (filename without the extension) to use as a container folder
        base_name, _ = os.path.splitext(file_name)

        # =================================================================
        # Define directory path with conditional subcategory and base_name
        # =================================================================
        if category_clean.lower() == "others":
            subcategory = str(gt_item.get("subcategory") or "Default").strip()
            subcategory_clean = subcategory.replace("/", "_").replace("\\", "_") if subcategory else "Default"
            
            # structure: source/category/subcategory/filename_folder/filename.pdf
            destination_path = f"{source_folder}/{category_clean}/{subcategory_clean}/{base_name}/{file_name}"
        else:
            # structure: source/category/filename_folder/filename.pdf
            destination_path = f"{source_folder}/{category_clean}/{base_name}/{file_name}"
        ...
        if pdf_bytes and blob_service_client and container_name:
            ...
            # =============================================================
            if langfuse_client:
                # Obtain the directory segment containing the PDF
                dest_dir = "/".join(destination_path.split("/")[:-1])
                prompts = _fetch_prompts_from_tabak_langfuse(file_name, langfuse_client)
